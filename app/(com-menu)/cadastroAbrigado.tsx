...
export default function CadastroAbrigado() {
  ...
  const [habilidades, setHabilidades] = useState({
    ...
    higienizarCozinha: false,
    cuidadosMedicos: false,
    auxiliarEnfermaria: false,
    apoioEmocional: false,
    acolhimentoInfantil: false,
    limpezaAmbientes: false,
    kitsHigiene: false,
    apoioAdm: false,
    organizacaoEstoques: false,
    organizacaoRoupas: false,
    trabalhoBracal: false,
  });
  ...
  const toggleHabilidade = (key) => {
    setHabilidades((prev) => ({ ...prev, [key]: !prev[key] }));
  };

  const toggleDoenca = (key) => {
    setDoencas((prev) => ({ ...prev, [key]: !prev[key] }));
  };
  ...
  const cadastrar = () => {
    ...
    console.log('Dados do voluntário:', {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {querSerVoluntario && (
        <>
          <Text style={styles.subtitulo}>Selecione as habilidades que você possui e está disposto(a) a oferecer durante o acolhimento:</Text>
          ...
          <Checkbox.Item label="Cuidados médicos" status={habilidades.cuidadosBasicos ? 'checked' : 'unchecked'} onPress={() => toggleHabilidade('cuidadosMedicos')} />
          <Checkbox.Item label="Auxiliar na enfermaria" status={habilidades.administrarMedicamentos ? 'checked' : 'unchecked'} onPress={() => toggleHabilidade('auxiliarEnfermaria')} />
          ...
          <Checkbox.Item label="Apoio administrativo" status={habilidades.organizacaoEstoques ? 'checked' : 'unchecked'} onPress={() => toggleHabilidade('apoioAdm')} />
      ...
      {possuiCondicao && (
        <>
          ...
          <Checkbox.Item
            label="Outra condição relevante"
            status={doencas.outra ? 'checked' : 'unchecked'}
            onPress={() => toggleDoenca('outra')}
          />
          {doencas.outra && (
            <TextInput
              style={styles.input}
